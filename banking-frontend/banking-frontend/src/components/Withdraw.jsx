...
const Withdraw = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:8080/accounts/withdraw', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        throw new Error('Withdrawal failed. Please check your account number or balance.');
      }

      const result = await res.json();
      setWithdrawAmount(parseFloat(formData.amount));
      setResponse(result);
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            ...
            <div style={{ 
              ...
              <p style={{ margin: '0', fontSize: '1.5rem', fontWeight: '600', color: 'var(--danger)' }}>
                -₹{withdrawAmount.toLocaleString('en-IN', { maximumFractionDigits: 2 })}
              </p>
            </div>
